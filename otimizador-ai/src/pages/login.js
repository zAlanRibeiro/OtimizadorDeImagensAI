...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    if (error) {
      setMessage({ type: 'error', content: "Email ou palavra-passe inválidos." });
    } else {
  ...
  return (
    <>
      <Head>
        <title>Login - Otimizador.AI</title>
      </Head>
      <main className="container mx-auto px-6 py-16 flex flex-col items-center text-center">
        <Breadcrumbs crumbs={crumbs} />
        <h1 className="text-4xl md:text-5xl font-bold">Acesse à sua conta</h1>
        <p className="text-lg text-slate-400 mt-4 max-w-2xl mx-auto">
          Estamos contentes por o ter de volta!
        </p>
        <div className="mt-8 w-full max-w-md">
          <form onSubmit={handleLogin} className="bg-slate-800/50 border border-slate-700 rounded-lg p-8 text-left space-y-4">
            {message.content && (
            ...
            <div>
              <div className="flex justify-between items-center">
                ...
                <a href="#" className="text-sm text-blue-400 hover:underline">Esqueceu-se?</a>
              </div>
              <div className="relative mt-1">
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required className="block w-full bg-slate-900/50 border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500 pr-10" placeholder="••••••••" />
            ...
            <button type="submit" className="w-full btn-primary py-3 !mt-6" disabled={loading}>{loading ? 'A entrar...' : 'Entrar'}</button>
          ...
          <p className="mt-6 text-slate-400">Ainda não tem uma conta?{' '}<Link href="/register" className="font-medium text-blue-400 hover:underline">Registre-se</Link></p>
        </div>
      </main>
    </>
  );
